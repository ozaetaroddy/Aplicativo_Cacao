<template>
    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        <button
          ...
          :disabled="loading"
          aria-label="Importar facturas desde TXT"
        >
          <i class="fas fa-file-import"></i>
          <span>Importar TXT</span>
        </button>
        <router-link to="/compras/nuevo" class="btn-primary">
            ...
            <tbody>
                ...
                <td colspan="9" class="empty-state-cell">
                  <div class="empty-state">
                    ...
                    <div class="empty-title">
                      {{ search || filtroTipo ? 'Sin resultados' : 'No hay compras registradas' }}
                    </div>
                    <div class="empty-text">
                      {{
                        search || filtroTipo
                          ? 'Ajusta los filtros o la búsqueda para encontrar compras'
                          : 'Registra tu primera compra para comenzar'
                      }}
                    </div>
                    <router-link
                      v-if="!search && !filtroTipo"
                      to="/compras/nuevo"
              ...
              <tr v-else v-for="c in compras" :key="c._id" class="compra-row">
                ...
                <td>
                  <div class="cliente-cell">
                    ...
                    <div class="cliente-info">
                      <div class="cliente-nombre" :title="c.proveedor?.nombre">
                ...
                <td>
                  <span
                    class="badge-tipo"
                    :class="c.tipo_compra === 'inventario' ? 'tipo-inventario' : 'tipo-gasto'"
                  >
                ...
                <td>
                  <span
                    class="badge-pago"
                    :class="c.estado_pago === 'pagado' ? 'badge-pago-ok' : 'badge-pago-pending'"
                  >
  ...
  <span v-else class="text-muted small">—</span>
                ...
                <td>
                  <div class="actions-cell">
                    <router-link
                      ...
                      title="Editar compra"
                      :aria-label="`Editar compra ${c.numero_factura}`"
                    >
                    ...
                    <button
                      ...
                      :aria-label="`Eliminar compra ${c.numero_factura}`"
                    >
                      <i class="fas fa-trash" :class="{ 'fa-spin': eliminandoId === c._id }"></i>
                    </button>
                  </div>
                </td>
              </tr>
        ...
        <div class="table-footer">
          <div class="footer-info">
            Mostrando <strong>{{ compras.length }}</strong> de <strong>{{ total.toLocaleString() }}</strong> registros
          </div>
          ...
          <div class="modal-body">
            <div class="alert alert-info">
              ...
              <div>
                ...
                Puede seleccionar individualmente si cada factura es de
                <strong>Inventario</strong> o <strong>Gasto</strong>.
              </div>
            </div>
            ...
            <div class="import-file-row">
              <div class="file-input-wrapper">
                <input
                  ...
                  :disabled="importando"
                />
                <label for="fileInput" class="file-input-label" :class="{ disabled: importando }">
              ...
              <div class="import-actions">
                <button
                  class="btn-import-action btn-outline"
                  @click="seleccionarTodos('inventario')"
                  :disabled="!lineas.length || importando"
                >
                  <i class="fas fa-boxes"></i> Todos Inventario
                </button>
                <button
                  class="btn-import-action btn-outline"
                  @click="seleccionarTodos('gasto')"
                  :disabled="!lineas.length || importando"
                >
                  <i class="fas fa-receipt"></i> Todos Gasto
                </button>
                <button
                  class="btn-import-action btn-primary"
            ...
            <div v-if="lineas.length > 0" class="import-preview">
              <div class="preview-header">
                ...
                <span class="preview-hint">
                  Máx. 500 líneas por importación
                </span>
              </div>
              <div class="preview-table-wrapper">
                <table class="table-modern preview-table">
                  ...
                  <tbody>
                    ...
                    <tr v-if="lineas.length > 100">
                      <td colspan="6" class="text-center text-muted small">
                        … y {{ lineas.length - 100 }} línea(s) más
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div v-if="resultadoImportacion" class="import-result">
              <div
                class="alert"
                :class="(resultadoImportacion.errores?.length || 0) > 0 ? 'alert-warning' : 'alert-success'"
              >
                <i
                  :class="
                ...
                ></i>
                <div>
                  <strong>{{ resultadoImportacion.importados || 0 }}</strong> factura(s) importada(s) correctamente.
                  <span v-if="resultadoImportacion.proveedoresCreados > 0">
                    <br>
                    <small>
  ...
  <small>
    ...
    {{ resultadoImportacion.retencionesCreadas }} retención(es) generada(s)
              ...
              <div v-if="(resultadoImportacion.errores?.length || 0) > 0" class="import-errors">
                <div
                  v-for="(err, idx) in resultadoImportacion.errores.slice(0, 10)"
                  :key="idx"
                  class="error-item"
                >
                ...
                <div
                  v-if="resultadoImportacion.errores.length > 10"
                  class="error-more"
                >
...
const formatHora = (f) =>
  f
    ? new Date(f).toLocaleTimeString('es-EC', {
        hour: '2-digit',
        minute: '2-digit'
      })
    : ''
...
const pedirConfirmacion = (opts = {}) => {
  return new Promise((resolve) => {
    ...
    if (!modalConfirm) {
      modalConfirm = new Modal(document.getElementById('modalConfirmCompra'), {
        backdrop: 'static'
      })
    }
    modalConfirm.show()
...
const cargar = async () => {
  ...
  try {
    ...
    const res = await api.request(`/compras?${params.toString()}`, {
      method: 'GET',
      skipLoader: true
    })
    if (unmounted) return
...
const pedirEliminar = async (c) => {
  if (eliminandoId.value === c._id) return

  const confirmado = await pedirConfirmacion({
    titulo: 'Eliminar compra',
    mensaje: `¿Eliminar la compra ${c.numero_factura || 'sin número'}?`,
    detalle:
      c.tipo_compra === 'inventario'
        ? 'El inventario será revertido automáticamente.'
  ...
  eliminandoId.value = c._id
  try {
    await api.request(`/compras/${c._id}`, {
      method: 'DELETE',
    ...
    if (codigo === 'COMPRA_CON_PAGOS') {
      toast.error(e.message || 'La compra tiene pagos registrados')
    } else if (codigo === 'COMPRA_CON_RETENCIONES') {
      toast.error(e.message || 'La compra tiene retenciones asociadas')
    } else if (codigo === 'PERIODO_CERRADO') {
      toast.error(e.message || 'El período está cerrado')
    } else if (codigo === 'COMPRA_NOT_FOUND') {
      toast.error('La compra ya no existe')
      await cargar()
    } else {
...
const abrirModalImportar = () => {
  if (!modalImportar) {
    modalImportar = new Modal(document.getElementById('modalImportar'), {
      backdrop: 'static'
    })
  }
  // Resetear estado
...
const cerrarModalImportar = () => {
  ...
  modalImportar?.hide()
  // Si hubo importación exitosa, refrescar
  if ((resultadoImportacion.value?.importados || 0) > 0) {
    cargar()
    cargarStats()
  }
}
  ...
  reader.onload = (e) => {
    try {
      ...
      if (dataLines.length > IMPORT_MAX_LINEAS) {
        toast.error(`El archivo tiene ${dataLines.length} líneas. Máximo: ${IMPORT_MAX_LINEAS}`)
        return
      }
      ...
      if (parsed.length === 0) {
        toast.error('No se pudieron procesar líneas válidas. Verifica el formato del archivo.')
        return
      }
...
const importarFacturas = async () => {
  ...
  try {
    ...
    toast.success(`Importación completada: ${data.importados || 0} facturas`)

    // Refrescar stats siempre
    cargarStats()

    // Si no hubo errores, cerrar automáticamente
    if ((data.errores?.length || 0) === 0) {
      setTimeout(() => {
        if (!unmounted && modalImportar) {
          modalImportar.hide()
          cargar()
        }
      }, 2000)
    ...
    if (codigo === 'DEMASIADAS_LINEAS') {
      toast.error(e.message || `Máximo ${IMPORT_MAX_LINEAS} líneas por importación`)
    } else if (codigo === 'PERIODO_CERRADO') {
      toast.error(e.message || 'Algunas líneas pertenecen a períodos cerrados')
    } else if (codigo === 'SIN_LINEAS') {
      toast.error('No se enviaron líneas para importar')
    } else {
...
onBeforeUnmount(() => {
  ...
  try { modalImportar?.hide() } catch { /* noop */ }
...
.btn-primary { background: linear-gradient(135deg, #e67e22, #d35400); color: #fff; box-shadow: 0 4px 12px rgba(230, 126, 34, 0.3); }
.btn-primary:hover { transform: translateY(-2px); color: #fff; box-shadow: 0 8px 20px rgba(230, 126, 34, 0.4); }
...
.search-box .clear-btn { position: absolute; right: 8px; background: none; border: none; color: var(--text-muted); cursor: pointer; padding: 4px; border-radius: 4px; transition: all var(--transition-fast); }
.search-box .clear-btn:hover { color: var(--danger); background: var(--bg-table-stripe); }
...
.action-icon-btn { width: 32px; height: 32px; border-radius: var(--radius-sm); border: 1.5px solid var(--border-color); background: var(--bg-card); color: var(--text-secondary); cursor: pointer; display: inline-flex; align-items: center; justify-content: center; font-size: 0.78rem; transition: all var(--transition-fast); padding: 0; text-decoration: none; }
...
.btn-action-danger:hover:not(:disabled) { background: #e74c3c; color: #fff; border-color: #e74c3c; }
...
.limit-select { padding: 6px 10px; background: var(--bg-card); border: 1.5px solid var(--border-color); border-radius: var(--radius-sm); font-size: 0.82rem; font-weight: 600; cursor: pointer; color: var(--text-primary); }
.btn-page { width: 32px; height: 32px; border-radius: var(--radius-sm); border: 1.5px solid var(--border-color); background: var(--bg-card); color: var(--text-secondary); cursor: pointer; transition: all var(--transition-fast); }
.btn-page:disabled { opacity: 0.4; cursor: not-allowed; }
.btn-page:hover:not(:disabled) { border-color: #e67e22; color: #e67e22; }
.page-info { padding: 0 8px; font-size: 0.82rem; font-weight: 600; color: var(--text-primary); font-variant-numeric: tabular-nums; }
.btn-refresh { padding: 8px 14px; border-radius: var(--radius-md); border: 1.5px solid var(--border-color); background: var(--bg-card); cursor: pointer; font-weight: 600; font-size: 0.82rem; color: var(--text-secondary); transition: all var(--transition-fast); }
.btn-refresh:hover:not(:disabled) { border-color: #e67e22; color: #e67e22; }
...
.empty-title { font-weight: 700; font-size: 1.05rem; color: var(--text-primary); }
.empty-text { font-size: 0.85rem; color: var(--text-muted); max-width: 380px; }
.btn-empty-action { display: inline-flex; align-items: center; gap: 8px; padding: 10px 22px; background: #e67e22; color: #fff; border-radius: var(--radius-md); font-weight: 600; text-decoration: none; cursor: pointer; border: none; font-family: inherit; transition: all var(--transition-fast); }
.btn-empty-action:hover { background: #d35400; color: #fff; }
...
.skeleton-circle { width: 36px; height: 36px; border-radius: 50%; background: var(--bg-table-stripe); flex-shrink: 0; }
...
.modal-header-import { background: linear-gradient(135deg, #e67e22, #d35400); color: #fff; border: none; }
.modal-header-import .modal-title { display: flex; align-items: center; gap: 10px; font-size: 1.1rem; color: #fff; margin: 0; }
...
.file-input-label:hover:not(.disabled) { border-color: #e67e22; color: #e67e22; background: rgba(230, 126, 34, 0.05); }
...
.preview-header { margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px; }
.preview-header h6 { font-size: 0.9rem; font-weight: 700; display: flex; align-items: center; gap: 8px; margin: 0; color: var(--text-primary); }
