<template>
  <div class="reporte-page">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">
          Listado de compras por rango de fechas, con totales y exportación
        </p>
      </div>
      <div class="header-actions">
        <button class="btn-secondary" @click="exportarExcel" :disabled="loading || compras.length === 0">
          <i class="fas fa-file-excel"></i> Excel
        </button>
        <button class="btn-danger" @click="exportarPDF" :disabled="loading || compras.length === 0">
    ...
    <div class="filters-card">
      <div class="filters-grid">
        <div class="form-field">
          ...
          <input id="rep-desde" type="date" class="form-input" v-model="desde" :disabled="loading" />
        </div>
        <div class="form-field">
          <label class="form-label" for="rep-hasta">Hasta</label>
          <input id="rep-hasta" type="date" class="form-input" v-model="hasta" :disabled="loading" />
        </div>
        <div class="quick-dates">
          <span class="quick-dates-label">Rápido:</span>
          <button v-for="q in quickDates" :key="q.label" type="button" class="quick-chip" :disabled="loading" @click="aplicarQuickDate(q)">
            {{ q.label }}
          </button>
        </div>
        <div class="filters-actions">
          <button class="btn-primary" @click="generar" :disabled="loading || !desde || !hasta">
    ...
    <!-- STATS -->
    <div v-if="compras.length > 0 && !loading" class="stats-grid">
      <div class="stat-card stat-card-info">
        <div class="stat-icon azul"><i class="fas fa-file-invoice"></i></div>
        <div class="stat-info">
          <div class="stat-value">{{ compras.length.toLocaleString() }}</div>
    ...
    <div class="card-cacao">
      <div class="card-header card-header-flex">
        <div>
          ...
          Compras
          <span v-if="!loading" class="header-count">{{ compras.length }}</span>
        </div>
      </div>
      <div class="card-body p-0">
        ...
        <div v-else-if="compras.length === 0" class="empty-block">
          <div class="empty-icon"><i class="fas fa-chart-bar"></i></div>
          <div class="empty-title">
            {{ consultado ? 'Sin compras en el período' : 'Selecciona fechas y genera el reporte' }}
          </div>
          <div class="empty-text">
            {{ consultado ? 'No se encontraron compras con los filtros aplicados' : 'Elige un rango de fechas y presiona "Generar"' }}
          </div>
        </div>
          ...
          <table class="table-modern">
            ...
            <tbody>
              <tr v-for="c in compras" :key="c._id">
                <td><div class="cell-fecha">{{ formatFecha(c.fecha_emision) }}</div></td>
                <td>
                  <div class="cell-cliente">
                    <div class="cliente-avatar">{{ inicial(c.proveedor?.nombre) }}</div>
                    <div class="cliente-nombre" :title="c.proveedor?.nombre">{{ c.proveedor?.nombre || 'N/A' }}</div>
                  </div>
                </td>
                <td><code class="doc-badge">{{ c.numero_factura || '—' }}</code></td>
                <td class="text-end"><span class="money-num">{{ formatCurrency(c.subtotal) }}</span></td>
                <td class="text-end"><span class="money-num">{{ formatCurrency(c.iva) }}</span></td>
                <td class="text-end"><span class="money-total">{{ formatCurrency(c.total) }}</span></td>
              </tr>
            </tbody>
            <tfoot>
...
const totalSubtotal = computed(() => roundTo2(compras.value.reduce((acc, c) => acc + (Number(c.subtotal) || 0), 0)))
const totalIva = computed(() => roundTo2(compras.value.reduce((acc, c) => acc + (Number(c.iva) || 0), 0)))
const totalTotal = computed(() => roundTo2(compras.value.reduce((acc, c) => acc + (Number(c.total) || 0), 0)))
...
const aplicarQuickDate = (q) => {
  const hoy = new Date()
  if (q.tipo === 'hoy') { desde.value = toISODate(hoy); hasta.value = toISODate(hoy) }
  else if (q.tipo === 'dias') { const i = new Date(hoy); i.setDate(i.getDate() - (q.dias || 0)); desde.value = toISODate(i); hasta.value = toISODate(hoy) }
  else if (q.tipo === 'mes') { desde.value = toISODate(new Date(hoy.getFullYear(), hoy.getMonth(), 1)); hasta.value = toISODate(hoy) }
  else if (q.tipo === 'anio') { desde.value = toISODate(new Date(hoy.getFullYear(), 0, 1)); hasta.value = toISODate(hoy) }
}

const generar = async () => {
  if (!desde.value || !hasta.value) { toast.warning('Selecciona ambas fechas'); return }
  if (desde.value > hasta.value) { toast.warning('"desde" no puede ser posterior a "hasta"'); return }
  ...
  try {
    ...
    const res = await api.request(`/reportes/compras?${params.toString()}`, { method: 'GET', loaderMessage: 'Generando reporte...' })
    if (unmounted) return
    const data = Array.isArray(res) ? res : (res?.data || [])
    compras.value = data
    fechaGeneracion.value = new Date().toLocaleString('es-EC', { dateStyle: 'medium', timeStyle: 'medium' })
    if (data.length === 0) toast.info('No hay compras en el período seleccionado')
  } catch (e) {
    if (unmounted) return
    const codigo = e?.codigo || e?.code
    if (codigo === 'RANGO_INVALIDO') toast.error(e.message || 'Rango inválido')
    else if (codigo === 'RANGO_DEMASIADO_LARGO') toast.warning(e.message || 'Rango demasiado largo')
    else toast.error('Error al generar reporte: ' + e.message)
    compras.value = []
  } finally { if (!unmounted) loading.value = false }
}

const exportarExcel = () => {
  if (compras.value.length === 0) return toast.warning('No hay datos para exportar')
  ...
  rows.push({ Fecha: 'TOTALES', Proveedor: '', 'Nº Factura': '', Subtotal: totalSubtotal.value, IVA: totalIva.value, Total: totalTotal.value })
  const ws = XLSX.utils.json_to_sheet(rows)
  ws['!cols'] = [{ wch: 14 }, { wch: 30 }, { wch: 18 }, { wch: 12 }, { wch: 12 }, { wch: 12 }]
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'Compras')
  XLSX.writeFile(wb, `reporte_compras_${desde.value}_${hasta.value}.xlsx`)
...
const exportarPDF = () => {
  ...
  doc.text('REPORTE DE COMPRAS', pageWidth / 2, 16, { align: 'center' })
  ...
  autoTable(doc, {
    ...
    body: compras.value.map(c => [
      formatFecha(c.fecha_emision), c.proveedor?.nombre || 'N/A', c.numero_factura || '—',
      roundTo2(c.subtotal).toFixed(2), roundTo2(c.iva).toFixed(2), roundTo2(c.total).toFixed(2)
    ]),
    foot: [['', '', 'TOTALES', totalSubtotal.value.toFixed(2), totalIva.value.toFixed(2), totalTotal.value.toFixed(2)]],
    theme: 'striped',
    headStyles: { fillColor: [230, 126, 34], textColor: 255, fontSize: 9 },
    footStyles: { fillColor: [230, 230, 230], textColor: [0, 0, 0], fontStyle: 'bold', fontSize: 9 },
    styles: { fontSize: 8 },
    columnStyles: { 0: { cellWidth: 25 }, 1: { cellWidth: 55 }, 2: { cellWidth: 35 }, 3: { cellWidth: 22, halign: 'right' }, 4: { cellWidth: 22, halign: 'right' }, 5: { cellWidth: 25, halign: 'right' } },
    margin: { left: 14, right: 14 }
  })
  doc.save(`reporte_compras_${desde.value}_${hasta.value}.pdf`)
  toast.success('PDF generado')
}

onMounted(() => aplicarQuickDate(quickDates[2]))
onBeforeUnmount(() => { unmounted = true })
</script>

<style scoped>
/* Mismo estilo que ReporteVentas.vue — copiar tal cual y cambiar .title-icon verde por naranja */
...
.title-icon-orange { background: linear-gradient(135deg, #e67e22, #d35400); box-shadow: 0 6px 16px rgba(230,126,34,0.3); }
...
.btn-primary:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(230,126,34,0.4); }
...
.form-input:focus { border-color: #e67e22; box-shadow: 0 0 0 4px rgba(230,126,34,0.15); background: var(--bg-card); }
...
.quick-chip:hover:not(:disabled) { border-color: #e67e22; color: #e67e22; background: rgba(230,126,34,0.06); }
...
.header-count { margin-left: 6px; padding: 2px 10px; background: rgba(230,126,34,0.15); color: #d35400; border-radius: var(--radius-full); font-size: 0.72rem; font-weight: 800; }
...
.cliente-avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #e67e22, #d35400); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 0.75rem; flex-shrink: 0; }
...
.spinner-lg { width: 44px; height: 44px; border: 4px solid var(--border-color); border-top-color: #e67e22; border-radius: 50%; animation: spin 0.8s linear infinite; }
