<template>
  <div class="bandeja-widget">
    ...
    <div v-if="loading" class="widget-state">
      <div class="spinner-sm spinner-compras"></div>
    </div>

    <!-- EMPTY -->
    <div v-else-if="compras.length === 0" class="widget-state">
    ...
    <div v-else class="bandeja-lista">
      <div
        ...
        role="button"
        tabindex="0"
        @keydown.enter="verCompra(compra)"
      >
        <div class="bandeja-icon" :class="claseIconoPago(compra.estado_pago)">
          <i class="fas fa-shopping-cart"></i>
        </div>
        <div class="bandeja-info">
          <div class="bandeja-titulo" :title="compra.numero_factura">
            {{ compra.numero_factura || 'Sin número' }}
          </div>
          <div class="bandeja-subtitulo" :title="compra.proveedor?.nombre">
            {{ compra.proveedor?.nombre || 'Proveedor no asignado' }}
          </div>
        </div>
        <div class="bandeja-monto">
          <div class="monto">${{ (Number(compra.total) || 0).toFixed(2) }}</div>
          <span class="badge-estado" :class="claseBadgePago(compra.estado_pago)">
            {{ etiquetaPago(compra.estado_pago) }}
          </span>
        </div>
      </div>
    </div>
...
const cargarCompras = async () => {
  ...
  try {
    const response = await api.request(
      '/compras?page=1&limit=8&sortBy=fecha_emision&sortDir=desc',
    ...
    if (!unmounted) console.warn('[BandejaComprasWidget] Error:', e)
  } finally {
