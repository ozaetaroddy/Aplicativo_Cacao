<template>
  <div class="chart-widget">
    ...
    <div v-else-if="sinDatos" class="chart-state chart-empty">
      ...
      <span>No hay datos de compras en los últimos 7 días</span>
    </div>
    <div v-show="!loading && !sinDatos" class="chart-canvas-wrap">
      <canvas ref="chartCanvas" aria-label="Gráfico de compras últimos 7 días"></canvas>
    </div>
  </div>
</template>
...
const sinDatos = computed(() => {
  const arr = comprasDiarias.value
...
const renderChart = () => {
  ...
  const data = Array.isArray(comprasDiarias.value) && comprasDiarias.value.length
    ? comprasDiarias.value.map(v => Number(v) || 0)
    : [0, 0, 0, 0, 0, 0, 0]
  ...
  try {
    ...
    console.error('[ComprasChartWidget] Error al renderizar:', err)
  }
}
...
onMounted(async () => {
  try {
    ...
    console.warn('[ComprasChartWidget] Error cargando estadísticas:', e)
  } finally {
