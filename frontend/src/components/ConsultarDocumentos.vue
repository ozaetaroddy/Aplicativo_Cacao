        ...
        <div class="modal-content">
          <div class="modal-header modal-header-doc">
            <div class="d-flex align-items-center gap-3">
              ...
              <div>
                <h5 class="modal-title mb-0">{{ getTipoDocLabel(documentoActual?.tipo_documento) }}</h5>
            ...
            <div v-if="documentoActual" class="documento-preview">
              ...
              <div class="doc-header">
                <div class="doc-header-left">
                  <div class="logo-empresa">
                    ...
                    <div>
                      <div class="empresa-nombre">{{ documentoActual.razon_social_emisor || "System Ozaet's Electronics" }}</div>
                ...
                <div class="doc-header-center">
                  <div class="doc-tipo-badge">{{ getTipoDocLabel(documentoActual.tipo_documento).toUpperCase() }}</div>
                  <div class="doc-numero-badge">Nº {{ documentoActual.numero_factura || documentoActual.numero_guia || 'N/A' }}</div>
                  <div class="doc-fecha"><i class="far fa-calendar"></i> {{ formatFecha(documentoActual.fecha_emision) }}</div>
                  <div class="ambiente-badge" :class="documentoActual.ambiente_sri === '2' ? 'amb-produccion' : 'amb-pruebas'">
                    {{ documentoActual.ambiente_sri === '2' ? '● PRODUCCIÓN' : '● PRUEBAS' }}
                  </div>
                </div>
              ...
              <div v-if="documentoActual.clave_acceso" class="clave-section">
                <div class="clave-left">
                  <div class="clave-label"><i class="fas fa-key me-1"></i> Clave de Acceso</div>
                  ...
                  <div class="autorizacion-info">
                    <div v-if="documentoActual.numero_autorizacion">
                      <strong>Nº Autorización:</strong> {{ documentoActual.numero_autorizacion }}
                    </div>
                    <div v-if="documentoActual.fecha_autorizacion">
                      <strong>Fecha Autorización:</strong> {{ formatFechaHora(documentoActual.fecha_autorizacion) }}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="cliente-section">
                <div class="cliente-box">
                  ...
                  <div class="cliente-row">
                    ...
                    <span class="cliente-row-value">{{ documentoActual.cliente?.nombre || documentoActual.proveedor?.nombre || 'Consumidor Final' }}</span>
                  </div>
                  <div class="cliente-row">
                    <span class="cliente-row-label">RUC/Cédula:</span>
                    <span class="cliente-row-value font-monospace">{{ documentoActual.cliente?.ruc || documentoActual.proveedor?.ruc || '9999999999999' }}</span>
                  </div>
                </div>
                <div class="cliente-box">
                  <div class="cliente-box-title"><i class="fas fa-info-circle me-1"></i> Datos del Documento</div>
                  <div class="cliente-row">
                ...
                <table class="detalles-table">
                  ...
                  <tbody>
                    <tr v-for="(item, idx) in documentoActual.detalles || []" :key="idx">
                      ...
                      <td class="text-right font-monospace">${{ (item.precio_unitario || item.costo_unitario || 0).toFixed(2) }}</td>
...
const abrirModal = () => {
  if (!modalInstance.value) {
    ...
    modalInstance.value = new Modal(modalEl, { backdrop: 'static', keyboard: false })
  }
...
const guardarPDF = () => {
  ...
  try {
    ...
    pdf.setFontSize(10)
    pdf.setFont('helvetica', 'normal')
    pdf.text(`Nº: ${doc.numero_factura || 'N/A'}`, pageWidth / 2, y, { align: 'center' })
    y += 6
    pdf.text(`Fecha: ${formatFecha(doc.fecha_emision)}`, pageWidth / 2, y, { align: 'center' })
    y += 12
    ...
    if (doc.clave_acceso) {
      ...
      chunks.forEach(c => { pdf.text(c, 14, y); y += 4; })
      y += 4
    }
    ...
    pdf.setFontSize(10)
    pdf.setFont('helvetica', 'bold')
    pdf.text(`Subtotal: $${(doc.subtotal || 0).toFixed(2)}`, pageWidth - 60, y); y += 5
    pdf.text(`IVA: $${(doc.iva || 0).toFixed(2)}`, pageWidth - 60, y); y += 6
    pdf.setFontSize(12)
    pdf.text(`TOTAL: $${(doc.total || 0).toFixed(2)}`, pageWidth - 60, y)
...
const verXml = async (doc) => {
  ...
  try {
    const res = await api.request(`/ventas/${doc._id}/xml-preview`, { method: 'GET' })
...
.doc-header {
  ...
  border-bottom: 3px double #1a3a5c; margin-bottom: 14px;
}
...
.logo-icon {
  ...
  display: flex; align-items: center; justify-content: center;
  color: #f1c40f; font-size: 1.4rem; font-weight: 800;
}
.empresa-nombre { font-size: 1.1rem; font-weight: 800; color: #1a3a5c; line-height: 1.1; }
...
.qr-img { width: 90px; height: 90px; border: 1px solid #e0e0e0; border-radius: 6px; padding: 4px; }
...
.cliente-box {
  ...
  border: 1px solid #d5dbe0; border-radius: 6px;
  background: #fafbfc;
}
.cliente-box-title {
  font-size: 0.7rem; color: #666; text-transform: uppercase;
  letter-spacing: 0.5px; font-weight: 700; margin-bottom: 6px;
  padding-bottom: 4px; border-bottom: 1px dashed #d5dbe0;
}
...
.detalles-title {
  font-size: 0.75rem; font-weight: 700; color: #1a3a5c;
  text-transform: uppercase; letter-spacing: 0.5px;
  padding: 6px 0; border-bottom: 2px solid #1a3a5c;
}
.detalles-table { width: 100%; border-collapse: collapse; font-size: 0.82rem; }
.detalles-table thead { background: #1a3a5c; color: #fff; }
.detalles-table thead th { padding: 8px; text-align: left; font-size: 0.72rem; text-transform: uppercase; }
...
.totales-box {
  ...
  border: 1px solid #d5dbe0; border-radius: 6px; overflow: hidden;
}
.total-row {
...
.total-final {
  display: flex; justify-content: space-between;
  padding: 14px; background: linear-gradient(135deg, #1a3a5c, #2c3e50);
...
.doc-footer {
  ...
  border-top: 2px solid #1a3a5c; text-align: center;
  font-size: 0.72rem; color: #666; line-height: 1.7;
}

.xml-viewer {
  background: #1e1e1e; border-radius: 10px; padding: 16px;
  max-height: 65vh; overflow: auto;
}
.xml-viewer pre {
  color: #d4d4d4; font-size: 0.78rem; line-height: 1.6;
...
@media (max-width: 768px) {
  ...
  .doc-header-center, .doc-header-right { text-align: left; }
