...
async function crearIndices(db) {
  const ops = [
    ...
    // ✅ NUEVO: unicidad de numero_factura por (tipo, ruc) — evita duplicados
    db.collection('ventas_v2').createIndex(
  ...
  return { total: ops.length, fallidos };
}

/**
 * Solo corre crearIndices si la versión de schema cambió.
 * Guarda la versión en la colección `meta`.
 */
